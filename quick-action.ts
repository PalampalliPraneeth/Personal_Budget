...
/* ---------------------------------------------------------------------
   ...
   --------------------------------------------------------------------- */
// BUGFIX: the hard-coded 95.0 fallback drifts from real FX over time and
// also didn't match every fallback value used elsewhere in this project
// (84 in one spot, 95 in others). If the FX API is briefly down, prefer the
// ledger's own last recorded rate — the same fallback Daily_Price_refresh.ts
// now uses — since that's always closer to reality than a fixed constant.
async function fetchUsdInrRate(DATA?: any): Promise<number> {
  ...
  return 95.0; // last resort only if the API is down AND no rate was ever recorded
}
...
// BUGFIX: this used to read bank.m[monthIdx] directly. In the app, a month
// with nothing typed into it isn't $0 — it carries forward the last month
// that DOES have a value (see core-data.js bankNativeValueAt/
// bankCarryValueAt), same as a real bank statement. Reading the raw cell
// meant a reminder sent on, say, the 2nd of a new month (before you've
// touched that account yet) reported "$0 available" and told you to "add"
// your entire balance again, even though the money was already there.
// Ported 1:1 from core-data.js so this matches what Overview/Cashflow show.
function bankNativeValueAt(bank: any, monthIdx: number): number | null {
  const arr = bank?.m;
  if (!arr) return null;
  for (let i = monthIdx; i >= 0; i--) {
    if (arr[i] !== null && arr[i] !== undefined) return Number(arr[i]) || 0;
  }
  return null;
}
function bankHasTransactionLog(bank: any): boolean {
  return !!(bank?.transactions?.length);
}
function bankDisplayValueAt(bank: any, monthIdx: number): number {
  const arr = bank?.m;
  if (arr && arr[monthIdx] !== null && arr[monthIdx] !== undefined) return Number(arr[monthIdx]) || 0;
  // Blank this month: only carry forward if this account actually has real
  // transaction history to justify it — otherwise it genuinely starts at $0,
  // matching bankCarryValueAt()'s rule in the client.
...
async function runReminderCheck(testMode = false) {
  try {
    ...
    for (const yearKey of Object.keys(DATA)) {
      ...
      for (const inv of DATA[yearKey]?.investments || []) {
        for (const h of inv.holdings || []) {
          ...
          allPlans.push({
            planId: `${yearKey}:${h.id}`, symbol: h.symbol || h.name || 'a holding', platform: inv.name || 'a platform',
    ...
    // BUGFIX: dedupeKey now includes the yearKey a plan came from, not just
    // its holding id. Two different holdings in two different years can
    // legitimately share the same id/UUID scheme once a year-rollover
    // feature exists; without the year prefix a reminder correctly sent
    // for one year's plan would silently suppress the other year's plan
    // forever (same key -> "already sent").
    const dueByLead: Record<number, DueItem[]> = { 7: [], 3: [], 1: [] };
    const dedupeKeyByLead: Record<number, string[]> = { 7: [], 3: [], 1: [] };
    ...
    for (const plan of allPlans) {
      ...
      dedupeKeyByLead[diffDays].push(dedupeKey);
    }
    ...
    const fxRate = await fetchUsdInrRate(DATA);
    ...
    // BUGFIX: previously every email in this run was sent first, and
    // notifiedDates was only written to storage ONCE at the very end. If
    // the 7-day email sent fine but the 3-day email's Resend call then
    // failed, the whole function threw, the final upsertKey() never ran,
    // and NEITHER dedupe key was persisted — so the 7-day email (which the
    // person already received) would be sent AGAIN on the next cron tick,
    // repeating every ~30 minutes until every bucket that run finally
    // succeeded in the same run. Now each bucket's dedupe key is persisted
    // immediately after that bucket's email is confirmed sent, so a later
    // failure can only affect buckets that haven't gone out yet.
    let sentCount = 0;
    ...
    for (const leadDays of activeLeadBuckets) {
      try {
        ...
        notifiedDates = notifiedDates.concat(dedupeKeyByLead[leadDays]).slice(-300);
        await upsertKey(NOTIFIED_KEY, notifiedDates);
      } catch (e) {
        firstError = firstError ?? e;
        console.error(`send-recurring-reminders: failed to send ${leadDays}-day reminder — will retry next run`, String(e));
      }
    }
    if (firstError && sentCount === 0) throw firstError; // nothing got through at all — surface it as a real failure
  } catch (e) {
